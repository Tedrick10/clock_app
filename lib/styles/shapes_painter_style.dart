...
// ShapesPainterStyle Style Class
class ShapesPainterStyle extends CustomPainter {
  @override
  void paint(Canvas canvas, Size size) {
    ...
    final double r =
        sqrt(size.width * size.width + size.width * size.height) / 2;
    ...
    double hoursDegree = 360 / 12 * (now.hour - 12); // Set hours to 12 hours format
    x = (size.width / 2) +
        (size.width / 3 - 50) * cos(Vector.radians(hoursDegree));
    y = (size.height / 2) +
        (size.height / 3 - 50) * sin(Vector.radians(hoursDegree));
